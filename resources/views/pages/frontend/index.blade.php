    ...
    <section class="flex items-center hero">
        <div class="w-full absolute z-20 inset-0 md:relative md:w-1/2 text-center flex flex-col justify-center hero-caption">
            ...
            <h2 class="px-8 text-base md:px-0 md:text-lg my-6 tracking-wide">
                Heug Coffee menawarkan konsep kedai kopi ala garasi mobil serta motor-motor klasik
                <br class="hidden lg:block" />dengan dekorasi vintage serta berjiwa petualang!
            </h2>
            <div>
                <a href="https://maps.app.goo.gl/XttTutMKUvcBZXAT8" target="_blank"
                    class="bg-pink-400 text-black hover:bg-black hover:text-pink-400 rounded-full px-8 py-3 mt-4 inline-block flex-none transition duration-200">Alamat
                    HEUG CAFFE</a>
            </div>
        </div>
            ...
            <div class="relative hero-image">
              ...
              <div class="absolute w-full h-full">
                <iframe
                  width="100%"
                  height="100%"
                  src="https://www.youtube.com/embed/MX59T6fUZyQ"
                  frameborder="0"
                ...
                <img src="/frontend/images/content/1.jpeg" alt="hero 1"
            ...
            <div class="flex -mx-4 flex-row relative">
                ...
                @foreach ($category as $categories)
                    <div class="px-4 relative card group">
                        <div class="rounded-xl overflow-auto card-shadow relative" style="width: 287px; height: 386px">
                            <div
                            ...
                            <img src="{{ $categories->galleri()->exists() ? Storage::url($categories->galleri->first()->url) : 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mN88B8AAsUB4ZtvXtIAAAAASUVORK5CYII=' }}"
                                alt="" class="w-full h-full object-cover object-center" />
                        </div>
                        <h5 class="text-lg font-semibold mt-4">{{ $categories->name }}</h5>
                        <a href="{{ route('categories-details', $categories->slug) }}"class="stretched-link">
                            <!-- fake children -->
                        </a>
                    </div>
                @endforeach
            ...
            <div class="flex -mx-4 flex-row relative">
                ...
                @foreach ($tables as $table)
                    <div class="px-4 relative card group">
                        ...
                        <h5 class="text-lg font-semibold mt-4">{{ $table->name }}</h5>
                        <span class="">
                            @if ($table->stock > 0)
                                SISA TABLE : {{ number_format($table->stock) }}
                            @endif
                        ...
                        <a href="{{ route('Table-Details', $table->slug) }}" class="stretched-link">
                            <!-- fake children -->
                        </a>
                    </div>
                @endforeach
